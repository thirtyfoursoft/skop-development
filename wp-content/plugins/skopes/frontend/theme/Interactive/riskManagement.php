...
//	include(RC_TC_BASE_DIR."/frontend/core/class_userinfo.php");
	...
	if (!empty( $getAllRiskManagementBlock )) {
		...
		foreach ($getAllRiskManagementBlock as $k => $v) { ?>
			<div class="riskBlock form-three-colum" id="riskBlock_<?=$k; ?>">
				<input type="hidden" class="riskStatus" value="<?php echo $v['id'];?>" name="risk[<?=$k; ?>][id]">
				<div class="first-colum"><input type="text" value="<?php echo $v['riskName']; ?>" name="risk[<?=$k; ?>][riskName]" class="required input-text" aria-required="true"></div> 
				...
				<div class="third-colum">
					...
					<?php if ( $k > 0 ) { ?>
						<a style="color: #ff0015 !important; display: inline-block; font-size: 18px; font-weight: bold; right: 30px; margin-top: 0px; float: right;"  href="javascript:void(0);" id="removeRiskBlock" onclick="removeRiskBlock('<?=$k; ?>',<?=$v['id']; ?>);">x</a>
					<?php } ?>					
				</div>
				<div class="full-colum">
					<textarea name="risk[<?=$k; ?>][riskDescription]" rows="4" cols="50" placeholder="Description" class="required input-textarea" aria-required="true"><?php echo $v['riskDescription']; ?></textarea>
				</div>
			</div>
	<?php
		}
