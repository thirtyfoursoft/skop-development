	   ...
	   e.preventDefault();
		   ...
		   if(forgotEmailID != '')
		   {
			 ...
			 $.ajax({
				...
				beforeSend: function(){
					$('#show_msg_a').html('<img src="/wp-content/plugins/skopes/frontend/theme/White/images/loader.gif" />');
				},
...
jQuery(function(){
	...
	jQuery(document).on('click', '#save_custom_cat', function (e) {
		...
		actionURL = jQuery("#actionURL").val()+"/skopes/frontend/theme/White/save_custom_cat.php?catname="+catname+"&catdesc="+catdesc+"&catIDUseID="+catIDUseID; 
		...
		if( res == 2 ) {
			...
			{ 
				 $.ajax({
						...
						beforeSend: function(){
							/* $('#abimg').html('<img src="/wp-content/plugins/skopes/frontend/theme/White/images/loader.gif" />');	 */			
						},
						success: function(data){
...
//		jQuery("#close_popup").click();
	...
	jQuery(document).on('click', '#com_noaccess,#pro_noaccess,#feature_noaccess', function (e) {	
		...
		jQuery("#small_popup").html("");
		jQuery(".popupbox").addClass("smallpopup");
		jQuery("#small_popup").load( "/wp-content/plugins/skopes/frontend/theme/White/msg.php?type=noaccress",function(data){
			jQuery("#popup").fadeIn(500);
			jQuery(".overlay").fadeIn(500);
			jQuery("#small_popup").show(); 
			jQuery("#close_popup").show();
...
$('#backToguideComplete').click(function(e) {
	...
	$.ajax({
		...
		beforeSend: function(){
			$('div#shimag').html('<img src="/wp-content/plugins/skopes/frontend/theme/White/images/loader.gif" />');
		},
		success: function(data) {
