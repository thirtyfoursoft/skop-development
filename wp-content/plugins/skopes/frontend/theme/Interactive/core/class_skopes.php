	...
	class clsSkopes {
		...
		function ActivateUser($key) {
			...
			if($res>0) { 
				...
				('".$userid."', 'selection_creteria_info', 0),('".$userid."', 'membermoreinfo', 0),('".$userid."', 'stakeholderinfo', 0),('".$userid."', 'benefitinfo', 0),('".$userid."', 'user_stage', 1), ('".$userid."', 'orgBasicInfo', 0), ('".$userid."', 'orgContactInfo', 0), ('".$userid."', 'orgMissionInfo', 0);");
				return true;
			}
			else
				return false;
		...
		function SaveDefaultToolData($userid , $forAdmin = null )
		{
			global $wpdb;
	  ...
	  /* get name of all selection criteria */
		...
		function getAllSelectionCriteriaData($id) {
			global $wpdb;
 
			$sSQL="select * FROM ".$wpdb->prefix."specdoc_selection_criteria WHERE scid=$id";
			$res = $wpdb->get_row($sSQL,ARRAY_A);
			return $res;

		}
		
		function updateSelectionCriteria( $data ) {
			global $wpdb;
			$sql = $wpdb->query("UPDATE " . $wpdb->prefix . "specdoc_selection_criteria SET sc_name = '" . mysql_real_escape_string($data['sc_name']) . "', percentage = '" . mysql_real_escape_string($data['percentage']) . "', helpText = '" . mysql_real_escape_string($data['helpText']) . "' WHERE scid = '" . $data['scid'] . "' ");
			
		}

		function insertSelectionCriteria( $data ) {

			global $wpdb;
			$sql = $wpdb->query("INSERT INTO " . $wpdb->prefix . "specdoc_selection_criteria SET sc_name = '" . mysql_real_escape_string($data['sc_name']) . "', percentage = '" . mysql_real_escape_string($data['percentage']) . "', helpText = '" . mysql_real_escape_string($data['helpText']) . "'");

		}

		function insertMilestoneName( $data ) {

			global $wpdb;
			$sql = $wpdb->query("INSERT INTO " . $wpdb->prefix . "specdoc_milestones SET milestone = '" . mysql_real_escape_string($data['milestone']) . "', days = '" . (int)$data['days'] . "', helpText = '" . mysql_real_escape_string($data['helpText']) . "'");
		...
		function getMilestoneName( $id ) {
			...
			$sSQL= $wpdb->get_row("SELECT * FROM ".$wpdb->prefix."specdoc_milestones WHERE id=$id", ARRAY_A);
			return $sSQL;
		}

		function updateMilestoneName( $data ) {

			global $wpdb;
			$sql = $wpdb->query("UPDATE " . $wpdb->prefix . "specdoc_milestones SET milestone = '" . mysql_real_escape_string($data['milestone']) . "', days = '" . (int)$data['days'] . "', helpText = '" . mysql_real_escape_string($data['helpText']) . "' WHERE id = '" . $data['id'] . "'");
