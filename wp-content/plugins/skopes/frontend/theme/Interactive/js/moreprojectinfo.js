	...
	for (var i = 0; i < benefits.length; i++){
		...
		if(benefits[i].value === ''){
		 $("#error_"+id).html('Enter Benefit'); 
			...
			$("#complete_benefit").removeClass("green").addClass("grey1");
		 return false;
		}
...
var BenefitsWrapper1   = $("#BenefitsWrapper1"); //Input boxes wrapper ID
	...
	if(BenefitsWrapper1.find('div.form-two-colum').size() >0){
		var y1 = BenefitsWrapper1.find('div.form-two-colum').size(); 
	}else{
		var y1 = 1;
	}
	//initlal text box count
	var BenefitMaxInputs = 19;
	...
	$(AddBenefitButton1).click(function (e)  //on add input button click
	{
        if(y1 <= BenefitMaxInputs) //max input box allowed
        {
            ...
            $(BenefitsWrapper1).append('<div class="form-two-colum"><div class="first-colum">Benefit'+AnFieldCount+'</div><div class="second-colum"><input type="text" value="" id="anbenefit_'+AnFieldCount+'" name="anbenefits[]" class="benefits input-text"><span id="error_anbenefit_'+AnFieldCount+'" class="error sherror"></span><a class="removeabclass" href="#" onclick="decrement();">×</a></div></div>');
	...
	$(AddBenefitButton).click(function (e)  //on add input button click
	{
        if(y <= BenefitMaxInputs) //max input box allowed
        {
            ...
            $(BenefitsWrapper).append('<div class="form-two-colum"><div class="first-colum">Benefit'+AnFieldCount+'</div><div class="second-colum"><input type="text" value="" id="anbenefit_'+AnFieldCount+'" name="anbenefits[]" class="benefits input-text"><span id="error_anbenefit_'+AnFieldCount+'" class="error sherror"></span><a class="removeabclass" href="#" onclick="decrement();">×</a></div></div>');
							 ...
							 $.ajax({ 
									...
									success: function(data){	
										...
										if(data == 2){ 
											...
											$("#complete_benefit").removeClass("grey1").addClass("green");
	...
	if(checkselectioncre()== false){
	   ...
	   //return false; 
		...
		$.ajax({ 
				...
				beforeSend: function(){
					$('#result').show();
					$('#result').html('<img src="/wp-content/plugins/skopes/frontend/theme/Interactive/images/loader.gif" />');	 			
				},
				success: function(data){	
					$('#result').hide();
					...
					if(data == 2){ 
						...
						$("#compelte_Cretria").removeClass("grey1").addClass("green");
						...
						$("#compelte_Cretria").removeClass("green").addClass("grey1");
	...
	**/
	$('#frmteammoreinfo').on('submit', function(e){
        ...
        $.ajax({
            ...
            beforeSend: function(){
			    $('#teammoreresult').show();
                $('#teammoreresult').html('<img src="/wp-content/plugins/skopes/frontend/theme/Interactive/images/loader.gif" />');				
            },
            success: function(data){
				...
				if(mcompleteval == 1){
					...
					$("#compltemembermoreinfo").removeClass("green").addClass("grey1");
				    ...
				    $.ajax({ 
								...
								success: function(data){	
									...
									if(data == 2){
										...
										$("#compltemembermoreinfo").removeClass("grey1").addClass("green");
...
function addMilestoneFields(){
	...
	var MilestoneInputs  = 17; //maximum input boxes allowed
	...
	if(y1 <= MilestoneInputs) //max input box allowed
        {
			...
			$('<div class="milestoneinfo"><div class="form-three-colum"><div class="first-colum">Milestone <span id="miles_'+MFieldCount+'">'+MFieldCount+'</span>:</div><div class="second-colum"><input type="text" name="milestone[]" class="input-text" value=""></div><div class="third-colum"><input type="text" id="datepicker'+MFieldCount+'" name="milestonedate[]" value="" class="pickdate input-text"><a href="#" class="removemclass">x</a></div></div></div>').insertBefore(lastdiv);
		    ...
		    success: function(data){
				...
				var mcompleteval = $('#frm_milestone').find('#mileval').val();
					...
					if(mcompleteval == 1){
						...
						$("#complte_milestone").removeClass("green").addClass("grey1");
...
function fnvalidate_milestone(){
  ...
  var endDate = $("#datepickerfinal").val();
  breakOut = false; 
  pastDate = false;
			       ...
			       $.ajax({ 
							...
							success: function(data){	
								...
								if(data == 2){
									...
									$("#complte_milestone").removeClass("grey1").addClass("green");
