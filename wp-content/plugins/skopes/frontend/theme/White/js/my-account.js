...
//			cc_number : "Please enter credit card number."
		...
		submitHandler: function(form) {
			...
			$.ajax({
				type:		'POST',
				url:		window.location.pathname+'wp-content/plugins/skopes/frontend/theme/White/processPayment.php',
				dataType: 'json',
...
*/
	$('input[type=radio]').live('change', function() {
		...
		$.ajax({
			type:		'POST',
			url:		window.location.pathname+'wp-content/plugins/skopes/frontend/theme/White/processPayment.php?action=loadForm&method='+paymentMethod,
			dataType: 'html',
			beforeSend: function() {
				$('#place_order').attr('disabled', true);
				$('#payment_forms').prepend('<div class="attention"><img src="wp-content/plugins/skopes/images/loading.gif" alt="" />&nbsp;Please wait ... </div>');
