	   ...
	   e.preventDefault();
		   ...
		   if(forgotEmailID != '')
		   {
			 ...
			 $.ajax({
				...
				beforeSend: function(){
					$('#show_msg_a').html('<img src="/wp-content/plugins/skopes/frontend/images/loader.gif" />');
				},
...
jQuery(function(){
	...
	jQuery(document).on('click', '#save_custom_cat', function (e) {
		...
		actionURL = jQuery("#actionURL").val()+"/skopes/frontend/save_custom_cat.php?catname="+catname+"&catdesc="+catdesc+"&catIDUseID="+catIDUseID; 
...
//		jQuery("#close_popup").click();
	...
	jQuery(document).on('click', '#com_noaccess,#pro_noaccess,#feature_noaccess', function (e) {	
		...
		jQuery("#small_popup").html("");
		jQuery(".popupbox").addClass("smallpopup");
		jQuery("#small_popup").load( "/wp-content/plugins/skopes/frontend/msg.php?type=noaccress",function(data){
			jQuery("#popup").fadeIn(500);
			jQuery(".overlay").fadeIn(500);
			jQuery("#small_popup").show(); 
			jQuery("#close_popup").show();
...
$('#backToguideComplete').click(function(e) {
	...
	$.ajax({
		...
		beforeSend: function(){
			$('div#shimag').html('<img src="/wp-content/plugins/skopes/frontend/images/loader.gif" />');
		},
		success: function(data) {
