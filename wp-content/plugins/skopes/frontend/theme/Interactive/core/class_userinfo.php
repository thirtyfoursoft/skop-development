...
/* Save user Milestone */
	...
	function getAllMilestoneName() {
		global $wpdb;

		$sql = $wpdb->get_results("SELECT * FROM " . $wpdb->prefix . "specdoc_milestones ORDER BY status");
		return $sql;
	}
		...
		global $wpdb;
		 ...
		 foreach($myrows as $key=>$value){
			 ...
			 /*  if(($value->meta_key == 'company_address') && ($this->checkcFieldStatus('company_question2') == 1)){
			  ...
			  if(($value->meta_key == 'comp_ques12')&&($this->checkcFieldStatus('company_question12') == 1)){
				  $comp_ques12 = trim($value->meta_value); 
				  $resultarray['comp_ques12']=$comp_ques12;
				  if(!empty($comp_ques12))
				   $compcount++;
			  }
			  
			  if(($value->meta_key == 'comp_ques13')&&($this->checkcFieldStatus('company_question13') == 1)){
				  $comp_ques13 = trim($value->meta_value); 
				  $resultarray['comp_ques13']=$comp_ques13;
				  if(!empty($comp_ques13))
				   $compcount++;
			  }

			  if(($value->meta_key == 'comp_ques14')&&($this->checkcFieldStatus('company_question14') == 1)){
				  $comp_ques14 = trim($value->meta_value); 
				  $resultarray['comp_ques14']=$comp_ques14;
				  if(!empty($comp_ques14))
				   $compcount++;
			  }

			  if(($value->meta_key == 'comp_ques15')&&($this->checkcFieldStatus('company_question15') == 1)){
				  $comp_ques15 = trim($value->meta_value); 
				  $resultarray['comp_ques15']=$comp_ques15;
				  if(!empty($comp_ques15))
				   $compcount++;
			  }

			  if(($value->meta_key == 'comp_ques16')&&($this->checkcFieldStatus('company_question16') == 1)){
				  $comp_ques16 = trim($value->meta_value); 
				  $resultarray['comp_ques16']=$comp_ques16;
				  if(!empty($comp_ques16))
				   $compcount++;
			  }
...
************* */
	...
	function getMoreInfoStatus( $userid ) {
		...
		if ($is_risk_management == 1 ) {
			...
			$result = $wpdb->get_var("SELECT count(*) FROM " . $wpdb->prefix . "save_status_at_stage2 WHERE userId = '" . $userid . "' AND form_name IN ('Selection criteria','Team responsibilities and effort', 'Non-financial benefits', 'Milestones', 'riskManagementMoreInfo') AND status != 'Complete' ");
			
		} else {

			$result = $wpdb->get_var("SELECT count(*) FROM " . $wpdb->prefix . "save_status_at_stage2 WHERE userId = '" . $userid . "' AND form_name IN ('Selection criteria','Team responsibilities and effort', 'Non-financial benefits', 'Milestones') AND status != 'Complete' ");
		}
	...
	function getMoreInfoStatusFromuserstatusinfo( $userid ) {
		...
		if ($is_risk_management == 1 ) {
			...
			$result = $wpdb->get_var("SELECT count(*) FROM " . $wpdb->prefix . "specdoc_userstatusinfo WHERE userid = '" . $userid . "' AND section_name IN ('selection_creteria_info','membermoreinfo', 'benefitinfo', 'milestonestatus', 'riskManagementMoreInfo') AND section_status != '2' ");
			
		} else {

			$result = $wpdb->get_var("SELECT count(*) FROM " . $wpdb->prefix . "specdoc_userstatusinfo WHERE userid = '" . $userid . "' AND section_name IN ('selection_creteria_info','membermoreinfo', 'benefitinfo', 'milestonestatus') AND section_status != '2' ");
		}

		return $result;
	}

	function getStatusOfOrganizationSection ( $userid ) {

		global $wpdb;
		$totalCompanyQues = $this->cquestioncount(); 
		$userCompanyQues  = $this->usercquestioncount_custom($userid);

		$query = $wpdb->get_var("SELECT count(*) FROM " . $wpdb->prefix . "specdoc_userstatusinfo WHERE userid = '" . $userid . "' AND section_name IN ('orgBasicInfo','orgContactInfo', 'orgMissionInfo') AND section_status != '2' ");

		if ( ($totalCompanyQues == $userCompanyQues) && ( $query == 0 ) ) {
			$result = 0;
		} else {
			$result = 1;
		}
